...
type REngine with
    ...
    member this.Library(name: string) = this.Evaluate(sprintf "library(%s)" name) |> ignore

    /// Require a library in current R environment.
    member this.Require(name: string) = this.Evaluate(sprintf "require(%s)" name) |> ignore

    /// Run the source code in a file.
    member this.Source(filename: string) = this.Evaluate(sprintf "source(%s)" filename)
...
let inline (?) (expr: SymbolicExpression) (mem: string) = expr.Member(mem)